...
export const FloretLibrary = () => {
    ...
    useEffect(() => {
        let filteredResults = allFlowers;

        //filter by name of flower if searchTerm Exists
        if (searchTerm) {
            filteredResults = filteredResults.filter(flower => flower.name.toLowerCase().includes(searchTerm.toLowerCase()))
        }
        ...
        //filter by color if colorSelection exists
        if (colorSelection) {
            filteredResults = filteredResults.filter(flower => flower.color.name === colorSelection)
        }

        setFilteredFlowers(filteredResults)
    }, [searchTerm, meaningSelection, colorSelection, allFlowers, allFlowerMeanings])
