...
export const Arrangement = ({ arrangementObj, user, fetchAndSetAllArrangements }) => {
    ...
    return <>
        <div className="max-w-sm bg-white border border-gray-300 rounded-lg shadow-lg overflow-hidden">
            {/* Image */}
            <img src={arrangementObj.img} alt="Dall-e-3 generated arrangement image" className="w-fit object-cover" />

            {/* Card Content */}
            <div className="p-4">
                <h3 className="text-xl font-semibold text-stone-800 text-center">{arrangementObj.name}</h3>
                <p className="text-stone-500 mt-2">
                    Meaning: {arrangementMeanings.map((arrangementMeaning) => arrangementMeaning.meaning.meaningTag).join(" ")}
                </p>
                {/* Button Container */}
                <div>
